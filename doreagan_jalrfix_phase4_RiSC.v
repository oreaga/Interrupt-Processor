...
module RiSC (clk, reset);
	...
	assign 	MUXpc_out =	(ifid_is_jalr) ? MUXop1_out :
				...
				(rfe_in_WB) ? MEMWB_pc__out :
				(exception_in_WB) ?  MEM__data2out :
				PC__out_plus1;
	...
	assign	IFID_instr__in = 	(Pstomp) ? `ZERO :
                                (TLB_miss1__out || (MEM__data1 === 16'hxxxx)) ? 16'b0 : MEM__data1;
	...
	wire	[3:0]	CTL6_out_rT =	(Pstall | ~ifid_writesRF) ? {kmode, 3'd0} : {kmode, IFID_rA};
	...
	assign	Pstomp =	(ifid_is_jalr | takenBranch);
	...
	assign	IDEX_op1__in =	MUXop1_out;
	assign	IDEX_op2__in =	MUXop2_out;

	assign	IDEX_exc__in =	CTL6_out_exc;
	assign	IDEX_pc__in =	IFID_pc__out;

	IDEX	idex_reg(.reset(res_D|x), .clk(clk_D), .IDEX_op0__in(IDEX_op0__in), .IDEX_op1__in(IDEX_op1__in),
    ...
    wire        idex_is_jalr = (IDEX_op__out == `JALR);

	assign	MUXalu1_out = 	(idex_is_lui | (idex_is_jalr & ~idex_is_tlbw)) ? IDEX_op0__out : IDEX_op1__out;
	...
	assign	EXMEM_pc__in = 		MUXrfe_out;
	...
	assign	TLB_asid2__in = (exmem_is_tlbw) ? EXMEM_ALUout__out[13:8] : asid;
	...
	always @(posedge clk) begin
        ...
        $display("MUXpc_out=%h",
            MUXpc_out);
        $display("Null Mem=%h", (MEM.dbus1 === 16'hxxxx));
